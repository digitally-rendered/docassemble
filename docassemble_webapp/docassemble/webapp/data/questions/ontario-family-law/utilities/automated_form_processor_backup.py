...
class AutomatedFormProcessor:
    ...
    def extract_fields(self, form_path: Path, form_config: FormConfiguration) -> List[Dict]:
        ...
        logger.info(f"Extracting fields from {form_path}")
        
        # Import the comprehensive parser
        from comprehensive_form_parser import PDFFormParser, WordFormParser
        
        if form_path.suffix.lower() == '.pdf':
            parser = PDFFormParser(str(form_path), form_config.form_number)
        else:
            parser = WordFormParser(str(form_path), form_config.form_number)
        
        fields = parser.extract_fields()
        
        # Convert to dict format
        field_dicts = [field.to_dict() for field in fields]
        
        logger.info(f"Extracted {len(field_dicts)} fields")
        return field_dicts
    ...
    def generate_yaml(self, form_config: FormConfiguration, fields: List[Dict]) -> Path:
        ...
        generator = AutomatedYAMLGenerator(form_config)
