/**
 * Playwright Test for Ontario Family Law Form 15B
 * Ontario Family Law Form 15B
...
// Test suite for Form 15B
test.describe('Form 15B - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_15B_complete.yml`);
    ...
    await fillFormSpecificFields_15B(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 15B Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 15B")')).toBeVisible();
  });
...
// Form-specific field tests for Form 15B
test.describe('Form 15B - Field Validations', () => {
  ...
  test('should accept valid date for if_you_are_asking_to_change_support_starting_on_a_date_before_today_please_answer_the_following', async ({ page }) => {
    const dateField = await page.$('input[name="if_you_are_asking_to_change_support_starting_on_a_date_before_today_please_answer_the_following"]');
    if (dateField) {
...
async function fillFormSpecificFields_15B(page) {
