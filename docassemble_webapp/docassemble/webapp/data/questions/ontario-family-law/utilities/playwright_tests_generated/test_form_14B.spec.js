/**
 * Playwright Test for Ontario Family Law Form 14B
 * Ontario Family Law Form 14B
...
// Test suite for Form 14B
test.describe('Form 14B - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_14B_complete.yml`);
    ...
    await fillFormSpecificFields_14B(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 14B Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 14B")')).toBeVisible();
  });
...
// Form-specific field tests for Form 14B
test.describe('Form 14B - Field Validations', () => {

  test('should accept valid date for next_scheduled_court_date_if_any', async ({ page }) => {
    const dateField = await page.$('input[name="next_scheduled_court_date_if_any"]');
    if (dateField) {
...
async function fillFormSpecificFields_14B(page) {
