/**
 * Playwright Test for Ontario Family Law Form 8A
 * Ontario Family Law Form 8A
...
// Test suite for Form 8A
test.describe('Form 8A - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_8A_complete.yml`);
    ...
    await fillFormSpecificFields_8A(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 8A Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 8A")')).toBeVisible();
  });
...
// Form-specific field tests for Form 8A
test.describe('Form 8A - Field Validations', () => {
  ...
  test('should accept valid date for first_name_on_the_day_before_the_marriage_date', async ({ page }) => {
    const dateField = await page.$('input[name="first_name_on_the_day_before_the_marriage_date"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for last_name_on_the_day_before_the_marriage_date', async ({ page }) => {
    const dateField = await page.$('input[name="last_name_on_the_day_before_the_marriage_date"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for gender_on_the_day_before_the_marriage_date', async ({ page }) => {
    const dateField = await page.$('input[name="gender_on_the_day_before_the_marriage_date"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for relationship_dates', async ({ page }) => {
    const dateField = await page.$('input[name="relationship_dates"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
...
async function fillFormSpecificFields_8A(page) {
