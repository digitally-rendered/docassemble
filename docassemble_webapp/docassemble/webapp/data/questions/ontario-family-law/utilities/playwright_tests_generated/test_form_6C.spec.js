/**
 * Playwright Test for Ontario Family Law Form 6C
 * Ontario Family Law Form 6C
...
// Test suite for Form 6C
test.describe('Form 6C - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_6C_complete.yml`);
    ...
    await fillFormSpecificFields_6C(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 6C Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 6C")')).toBeVisible();
  });
...
// Form-specific field tests for Form 6C
test.describe('Form 6C - Field Validations', () => {
  ...
  test('should validate email_address email format', async ({ page }) => {
...
async function fillFormSpecificFields_6C(page) {
