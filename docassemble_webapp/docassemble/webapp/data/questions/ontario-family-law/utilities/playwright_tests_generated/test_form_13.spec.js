/**
 * Playwright Test for Ontario Family Law Form 13
 * Ontario Family Law Form 13
...
// Test suite for Form 13
test.describe('Form 13 - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_13_complete.yml`);
    ...
    await fillFormSpecificFields_13(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 13 Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 13")')).toBeVisible();
  });
...
// Form-specific field tests for Form 13
test.describe('Form 13 - Field Validations', () => {
  ...
  test('should accept valid date for i_attach_proof_of_my_yeartodate_income_from_all_sources_including_my_most_recent_attach_all_that_are_applicable', async ({ page }) => {
    const dateField = await page.$('input[name="i_attach_proof_of_my_yeartodate_income_from_all_sources_including_my_most_recent_attach_all_that_are_applicable"]');
    if (dateField) {
  ...
  test('should accept valid date for note_this_financial_statement_must_be_updated_before_any_court_event_if_it_is', async ({ page }) => {
    const dateField = await page.$('input[name="note_this_financial_statement_must_be_updated_before_any_court_event_if_it_is"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for you_may_update_this_financial_statement_by_either_completing_and_filing', async ({ page }) => {
    const dateField = await page.$('input[name="you_may_update_this_financial_statement_by_either_completing_and_filing"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
...
async function fillFormSpecificFields_13(page) {
