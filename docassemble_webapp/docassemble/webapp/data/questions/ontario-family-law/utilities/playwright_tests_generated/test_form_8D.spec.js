/**
 * Playwright Test for Ontario Family Law Form 8D
 * Ontario Family Law Form 8D
...
// Test suite for Form 8D
test.describe('Form 8D - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_8D_complete.yml`);
    ...
    await fillFormSpecificFields_8D(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 8D Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 8D")')).toBeVisible();
  });
...
// Form-specific field tests for Form 8D
test.describe('Form 8D - Field Validations', () => {
...
async function fillFormSpecificFields_8D(page) {
