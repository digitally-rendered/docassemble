/**
 * Playwright Test for Ontario Family Law Form 13A
...
// Test suite for Form 13A
test.describe('Form 13A - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_13A_complete.yml`);
    ...
    await fillFormSpecificFields_13A(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 13A Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 13A")')).toBeVisible();
  });
...
// Form-specific field tests for Form 13A
test.describe('Form 13A - Field Validations', () => {
...
async function fillFormSpecificFields_13A(page) {
