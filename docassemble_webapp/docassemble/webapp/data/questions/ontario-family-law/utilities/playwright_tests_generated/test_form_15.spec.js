/**
 * Playwright Test for Ontario Family Law Form 15
 * Ontario Family Law Form 15
...
// Test suite for Form 15
test.describe('Form 15 - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_15_complete.yml`);
    ...
    await fillFormSpecificFields_15(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 15 Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 15")')).toBeVisible();
  });
...
// Form-specific field tests for Form 15
test.describe('Form 15 - Field Validations', () => {

  test('should validate email email format', async ({ page }) => {
  ...
  test('should accept valid date for child_birthdate', async ({ page }) => {
    const dateField = await page.$('input[name="child_birthdate"]');
    if (dateField) {
  ...
  test('should accept valid date for date_parties_started_living_together__write_na_if_not_applicable', async ({ page }) => {
    const dateField = await page.$('input[name="date_parties_started_living_together__write_na_if_not_applicable"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for date_parties_married__write_na_if_not_applicable', async ({ page }) => {
    const dateField = await page.$('input[name="date_parties_married__write_na_if_not_applicable"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for date_parties_separated__write_na_if_not_applicable', async ({ page }) => {
    const dateField = await page.$('input[name="date_parties_separated__write_na_if_not_applicable"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for requesting_partys_birth_date', async ({ page }) => {
    const dateField = await page.$('input[name="requesting_partys_birth_date"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for responding_partys_birth_date', async ({ page }) => {
    const dateField = await page.$('input[name="responding_partys_birth_date"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for if_you_are_asking_to_change_support_starting_on_a_date_before_today__please_answer_the_following', async ({ page }) => {
    const dateField = await page.$('input[name="if_you_are_asking_to_change_support_starting_on_a_date_before_today__please_answer_the_following"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
  ...
  test('should accept valid date for you_must_file_all_your_completed_documents_with_the_court_so_a_court_clerk_can_sign_and_date_this_motion_to_change__you_can_file_documents_in_person_at_a_courthouse_or_online_by_visiting_wwwontariocafamilyclaims__you_must_then', async ({ page }) => {
    const dateField = await page.$('input[name="you_must_file_all_your_completed_documents_with_the_court_so_a_court_clerk_can_sign_and_date_this_motion_to_change__you_can_file_documents_in_person_at_a_courthouse_or_online_by_visiting_wwwontariocafamilyclaims__you_must_then"]');
    if (dateField) {
      await dateField.fill('01/01/2024');
      await page.click('button:has-text("Continue")');
    }
...
async function fillFormSpecificFields_15(page) {
