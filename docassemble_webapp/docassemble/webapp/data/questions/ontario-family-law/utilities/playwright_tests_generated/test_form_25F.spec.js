/**
 * Playwright Test for Ontario Family Law Form 25F
 * Ontario Family Law Form 25F
...
// Test suite for Form 25F
test.describe('Form 25F - Complete Interview', () => {
  test.setTimeout(TIMEOUT);
  
  test('should complete the entire form with valid data', async ({ page }) => {
    // Navigate to the interview
    await page.goto(`${INTERVIEW_URL}/form_25F_complete.yml`);
    ...
    await fillFormSpecificFields_25F(page);
    
    // Continue through remaining questions
    await continueToEnd(page);
    
    // Verify we reached the final screen
    await expect(page.locator('h1')).toContainText('Form 25F Complete');
    
    // Verify download button is present
    await expect(page.locator('button:has-text("Download Form 25F")')).toBeVisible();
  });
...
// Form-specific field tests for Form 25F
test.describe('Form 25F - Field Validations', () => {
});

// Helper functions for Form 25F
...
async function fillFormSpecificFields_25F(page) {
