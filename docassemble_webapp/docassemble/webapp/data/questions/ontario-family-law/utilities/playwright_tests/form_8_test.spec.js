/**
 * Playwright tests for Ontario Family Law Form 8
 * Generated from YAML interview: generated_interviews/form_8_enhanced.yml
 * Generated on: 2025-09-05 13:25:01
 * 
 * Title: Ontario Form 8 - Application (General)
 */
...
const testData = {
  ...
  'claims.orders_requested': 'Test Value',
  'claims.other_orders': 'Test Other orders',
};
...
test.describe('Form 8 - Main Flow', () => {
  test('Complete form with all required fields', async ({ page }) => {
    // Navigate to the interview
    await page.goto('/interview?i=docassemble.ontarioforms:data/questions/form_8.yml');
    ...
    await page.fill('[name="area"]', testData['area'] || 'Test Value');
    // Fill fields for: What orders are you asking the court to make?
    // TODO: Fill field claims.orders_requested (type: checkboxes)
    // TODO: Fill field [{'divorce': 'A divorce'}, {'sole_custody': 'Sole decision-making responsibility for children (to me)'}, {'joint_custody': 'Joint decision-making responsibility for children'}, {'primary_residence': 'Primary residence of children (with me)'}, {'child_support': 'Child support'}, {'spousal_support': 'Spousal support'}, {'property_division': 'Division of property'}, {'restraining_order': 'Restraining order'}, {'other': 'Other (specify below)'}] (type: checkboxes)
    await page.fill('[name="claims.other_orders"]', testData['claims.other_orders'] || 'Test Value');
    await page.fill('[name="claims.orders_requested.any_true('other')"]', testData['claims.orders_requested.any_true('other')'] || 'Test Value');
    // Verify completion
    await expect(page.locator('h1')).toContainText('Complete');
  });
});

test.describe('Form 8 - Field Validation', () => {
...
test.describe('Form 8 - Navigation', () => {
...
test.describe('Form 8 - Edge Cases', () => {
...
// Performance tests
test.describe('Form 8 - Performance', () => {
  test('Measure page load time', async ({ page }) => {
    ...
    console.log(`Form 8 loaded in ${loadTime}ms`);
  });
});
